...
filename = "c:/xd/schema_xml.xsd" 

# Request XML from localhost
rm = requests.get("http://localhost:3000/countries_male/")
rf = requests.get("http://localhost:3000/countries_female/")
rh = requests.get("http://localhost:3000/countries_groupby/")

# Get content from XML
root_male = ElementTree.fromstring(rm.content)
root_female = ElementTree.fromstring(rf.content)
root_happy = ElementTree.fromstring(rh.content)
...
if(xmlschema.validate(root_male) == True) and (xmlschema.validate(root_female) == True) and (xmlschema.validate(root_happy) == True) :
    ...
    fig = go.Figure(data=[ 
        go.Bar(name='Male', x=[x['Country'] for x in maleArray], y=[y['Suicide_rate'] for y in maleArray]),
        go.Bar(name='Female', x=[x['Country'] for x in femaleArray], y=[x['Suicide_rate'] for x in femaleArray])
    ])

    fig2 = go.Figure(data=[
        go.Bar(name='Happiness Index', x=[x['Country'] for x in happyArray], y=[y['Score'] for y in happyArray])
    ])
